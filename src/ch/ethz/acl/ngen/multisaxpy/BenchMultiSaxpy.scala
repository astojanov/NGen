...
class BenchMultiSaxpy extends Bench.ForkedTime {
  //
  ...
  performance of "SAXPY" config (
    ...
    measure method "jSaxpy (JVM implementation)" in {
      using(scalaArguments) in {
        case (a, b, scalar, size) => MultiSaxpy.jSaxpy(a, b, scalar, size)
      }
    }
    measure method "nSaxpy (LMS generated)" in {
      using(scalaArguments) in {
        case (a, b, scalar, size) => MultiSaxpy.nSaxpy(a, b, scalar, size)
      }
    }
  }
  //
